...
install_files() {
	...
	cp "$PKG_DIR/root/usr/libexec/openclaw-permissions.sh" "$dest/usr/libexec/"
	chmod +x "$dest/usr/libexec/openclaw-paths.sh" "$dest/usr/libexec/openclaw-node.sh" "$dest/usr/libexec/openclaw-compat.sh" "$dest/usr/libexec/openclaw-wechat.sh" "$dest/usr/libexec/openclaw-permissions.sh"
...
for dep in luci-compat luci-base curl openssl-util script-utils tar libstdcpp; do
