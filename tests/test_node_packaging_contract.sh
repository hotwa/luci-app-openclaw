...
grep -Fq 'NODE_VERSION_V2="22.23.2"' "$ENV_SCRIPT" || fail "installer should default V2 to Node.js 22.23.2"
APP_VERSION=$(tr -d '[:space:]' < "$VERSION_FILE")
[ "$APP_VERSION" = "2.1.1" ] || fail "package VERSION should track the upstream plugin release"
grep -Fq "## [${APP_VERSION}]" "$REPO_ROOT/CHANGELOG.md" || fail "changelog should include the hotwa release version used by VERSION"
grep -Fq 'OC_TESTED_VERSION="2026.7.1-2"' "$ENV_SCRIPT" || fail "stable OpenClaw runtime target should be pinned"
grep -Fq 'OC_NODE_MIN_VERSION="${OC_NODE_MIN_VERSION:-22.22.3}"' "$ENV_SCRIPT" || fail "installer should pin OpenClaw 2026.7.x minimum Node.js version"
grep -Fq 'OC_VERSION  — 指定 OpenClaw 版本 (如 2026.7.1)，不设置则安装已测试稳定版' "$ENV_SCRIPT" || fail "installer usage should describe pinned stable installs"
...
grep -Fq 'OC_NODE_MIN_VERSION="${OC_NODE_MIN_VERSION:-22.22.3}"' "$INIT_SCRIPT" || fail "init script should pin OpenClaw 2026.7.x minimum Node.js version"
...
grep -Fq "description: 'Build V2 (22.23.2) - OpenClaw 2026.7.1 compatible'" "$WORKFLOW" || fail "workflow should describe the OpenClaw-compatible V2 runtime"
grep -Fq 'NODE_VER="22.23.2"' "$WORKFLOW" || fail "workflow should request Node.js 22.23.2 for V2"
...
grep -Fq 'alpine:3.21 sh /build-node-musl.sh' "$WORKFLOW" || fail "workflow should build V2 from a pinned Alpine release"
grep -Fq 'EXPECTED_ARTIFACT="node-v22.23.2-linux-arm64-musl.tar.xz"' "$WORKFLOW" || fail "workflow should require the exact Node.js 22.23.2 V2 artifact name"
grep -Fq '`node-v22.23.2-linux-arm64-musl.tar.xz`' "$WORKFLOW" || fail "release notes should mention the Node.js 22.23.2 ARM64 musl tarball"
