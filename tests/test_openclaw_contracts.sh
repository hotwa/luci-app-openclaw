...
grep -q "local q_install_path = shell_quote(install_path)" luasrc/controller/openclaw.lua || fail "uninstall rm must shellquote install_path"
...
grep -q "openclaw-wechat.sh" luasrc/controller/openclaw.lua || fail "controller must use the shared wechat helper"
grep -q "start-stop-daemon -S -c openclaw" luasrc/controller/openclaw.lua || fail "wechat helper must run as the openclaw user"
grep -q "openclaw-paths.sh" root/usr/libexec/openclaw-wechat.sh || fail "wechat helper must use shared runtime paths"
grep -q "@tencent-weixin/openclaw-weixin-cli install" root/usr/libexec/openclaw-wechat.sh || fail "wechat helper must use the official installer"
grep -q "oc_apply_runtime_compat" root/usr/libexec/openclaw-wechat.sh || fail "wechat helper must repair runtime compatibility"
...
grep -q "compare_plugin_versions" luasrc/controller/openclaw.lua || fail "semantic version compare helper missing"
grep -q "compare_plugin_versions(plugin_latest, plugin_current) > 0" luasrc/controller/openclaw.lua || fail "plugin update check must not treat newer local versions as upgradeable"
grep -q "compare_plugin_versions(latest_version, current_version) > 0" luasrc/controller/openclaw.lua || fail "wechat update check must use semantic version compare"
...
grep -q "for dep in luci-compat luci-base curl openssl-util script-utils tar libstdcpp" scripts/build_run.sh || fail ".run installer must install runtime dependencies"
