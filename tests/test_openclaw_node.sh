...
oc_node_version_ge "22.23.2" "22.23.2" || fail "exact version should satisfy requirement"
oc_node_version_ge "22.23.3" "22.23.2" || fail "newer patch should satisfy requirement"
oc_node_version_ge "23.0.0" "22.23.2" || fail "newer major should satisfy requirement"
if oc_node_version_ge "22.23.1" "22.23.2"; then
...
cat > "$tmpdir/node-bins-release.json" <<'EOF'
{
  ...
  "assets": [
    ...
    {
      "name": "node-v22.23.2-linux-arm64-musl.tar.xz",
      "browser_download_url": "https://github.com/hotwa/luci-app-openclaw/releases/download/node-bins/node-v22.23.2-linux-arm64-musl.tar.xz"
    },
    {
...
selected_url=$(oc_select_node_release_asset_url "$tmpdir/node-bins-release.json" "linux-arm64" "22.23.2") || fail "select compatible ARM64 musl asset"
[ "$selected_url" = "https://github.com/hotwa/luci-app-openclaw/releases/download/node-bins/node-v22.23.2-linux-arm64-musl.tar.xz" ] || fail "selected asset should be newest compatible ARM64 musl release"
...
mirror_selected_url=$(oc_select_node_release_asset_url "$tmpdir/mirror-node-bins-release.json" "linux-arm64" "22.23.2") || fail "select compatible ARM64 musl asset from mirror release JSON"
[ "$mirror_selected_url" = "https://github.com/hotwa/luci-app-openclaw/releases/download/node-bins/node-v22.23.2-linux-arm64-musl.tar.xz" ] || fail "selected mirror asset should preserve browser_download_url"

if oc_select_node_release_asset_url "$tmpdir/node-bins-release.json" "linux-arm64" "22.23.3" >/dev/null 2>&1; then
